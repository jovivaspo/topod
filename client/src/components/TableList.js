...
const TableList = ({ podcasts }) => {
  ...
  const { setAlert, setLoading, working, setWorking } =
        ...
        <TableBody>
          {podcasts.map((el, index) => (
            <TableRow
              ...
              style={{
                backgroundColor:
                  audioPlayer?.currentSong?.id === el.podcastId
                    ? "rgba(205,167,87)"
                    : "transparent",
            ...
            >
              ...
              >
                {<img src={el.img} height="40" width="50" />}{" "}
                <h5>{el.title}</h5>
              </TableCell>
              <TableCell className={classes.cell} align="right">
                <Button
                  onClick={() =>
                    handlerSong(el.podcastId, el.title, el.duration)
                  }
                >
                  {audioPlayer?.currentSong?.id === el.podcastId &&
              ...
              <TableCell className={classes.cell} align="right">
                <Button onClick={() => handlerDownload(el.podcastId, el.title)}>
                  <GetAppIcon />
                </Button>
              </TableCell>
              <TableCell className={classes.cell} align="right">
                <Button onClick={() => handlerDelete(el.id, el.podcastId)}>
                  <DeleteForeverIcon />
                </Button>
              </TableCell>
              <TableCell className={classes.cell} align="right">
                {secondsToString(el.duration)}
              </TableCell>
              <TableCell className={classes.cell} align="right">
                {timeAgo(el.date, "es")}
              </TableCell>
